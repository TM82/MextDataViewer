...
// --- Elements & State ---
let folderSel, fileSel, rowSel, colSel, tableEl;
...
async function onFolder(){
  ...
  for(const f of ds.files){
    const title = f.title || (f.path ? (f.path.split('/').pop() || 'CSV') : 'CSV');
    const opt = document.createElement('option');
    opt.value = f.path || '';
...
async function onFile(){
  ...
  try{
    ...
    setStatus('CSV読込エラー：' + e.message + '（パス：' + relPath + '）');
  }
}
...
function onFilterChange(){
  ...
  if(rowVal && !colVal){
    ...
    lastFiltered = out; applySort(); return;
  }
  if(!rowVal && colVal){
...
// init
function init(){
  folderSel = document.getElementById('folderSelect');
  fileSel   = document.getElementById('fileSelect');
  rowSel    = document.getElementById('rowSelect');
  colSel    = document.getElementById('colSelect');
  tableEl   = document.getElementById('dataTable');

  if(!folderSel || !fileSel || !rowSel || !colSel || !tableEl){
    setStatus('初期化エラー: 必要な要素が見つかりません。index.htmlのIDを確認してください。');
    return;
  }
